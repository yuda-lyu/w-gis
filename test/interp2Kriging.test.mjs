...
describe(`interp2Kriging`, function() {
    let k = -1
    let oin = {}
    let out = {}
    ...
    it(`should return variogram when interp2Kriging(returnWithVariogram=true)`, function() {
        k = 8
        let r = interp2Kriging(oin[k].ps, oin[k].p, oin[k].opt)
        delete r.variogram.model
        delete r.variogram.svpd
        assert.strict.deepStrictEqual(r, out[k])
    })
