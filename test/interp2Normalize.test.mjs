import assert from 'assert'
import interp2Normalize from '../src/interp2Normalize.mjs'


describe(`interp2Normalize`, function() {
    let k = -1
    let oin = {}
    let out = {}

    let ps = [{ x: 243, y: 206, z: 95 }, { x: 233, y: 225, z: 146 }, { x: 21, y: 325, z: 22 }, { x: 953, y: 28, z: 223 }, { x: 1092, y: 290, z: 39 }, { x: 744, y: 200, z: 191 }, { x: 174, y: 3, z: 22 }, { x: 537, y: 368, z: 249 }, { x: 1151, y: 371, z: 86 }, { x: 814, y: 252, z: 125 }]

    k++
    oin[k] = { ps, opt: {} }
    out[k] = {
        ps: [{ x: 0.19646017699115045, y: 0.5168141592920354, z: 0.32158590308370044, ind: null }, { x: 0.18761061946902655, y: 0.5336283185840708, z: 0.5462555066079295, ind: null }, { x: 0, y: 0.6221238938053097, z: 0, ind: null }, { x: 0.8247787610619469, y: 0.35929203539823007, z: 0.8854625550660793, ind: null }, { x: 0.947787610619469, y: 0.5911504424778761, z: 0.07488986784140969, ind: null }, { x: 0.6398230088495576, y: 0.511504424778761, z: 0.7444933920704846, ind: null }, { x: 0.13539823008849558, y: 0.33716814159292036, z: 0, ind: null }, { x: 0.45663716814159294, y: 0.6601769911504425, z: 1, ind: null }, { x: 1, y: 0.6628318584070797, z: 0.28193832599118945, ind: null }, { x: 0.7017699115044248, y: 0.5575221238938053, z: 0.45374449339207046, ind: null }],
        psMinMax: { xmin: 0, xmax: 1, ymin: 0.33716814159292036, ymax: 0.6628318584070797, zmin: 0, zmax: 1 },
        st: { x: { min: 21, max: 1151, range: 1130 }, y: { min: -378, max: 752, range: 1130 }, z: { min: 22, max: 249, range: 227 } },
    }
    it(`should normalize points when interp2Normalize(default scale=1)`, function() {
        k = 0
        let r = interp2Normalize(oin[k].ps, oin[k].opt)
        delete r.nv
        delete r.inv
        assert.strict.deepStrictEqual(r, out[k])
    })

    k++
    oin[k] = { ps, opt: { scale: 100 } }
    out[k] = {
        ps: [{ x: 19.646017699115045, y: 51.681415929203546, z: 32.158590308370044, ind: null }, { x: 18.761061946902654, y: 53.36283185840708, z: 54.62555066079295, ind: null }, { x: 0, y: 62.21238938053097, z: 0, ind: null }, { x: 82.47787610619469, y: 35.92920353982301, z: 88.54625550660793, ind: null }, { x: 94.77876106194691, y: 59.11504424778761, z: 7.488986784140969, ind: null }, { x: 63.982300884955755, y: 51.1504424778761, z: 74.44933920704845, ind: null }, { x: 13.539823008849558, y: 33.716814159292035, z: 0, ind: null }, { x: 45.663716814159294, y: 66.01769911504425, z: 100, ind: null }, { x: 100, y: 66.28318584070797, z: 28.193832599118945, ind: null }, { x: 70.17699115044248, y: 55.75221238938053, z: 45.37444933920705, ind: null }],
        psMinMax: { xmin: 0, xmax: 100, ymin: 33.716814159292035, ymax: 66.28318584070797, zmin: 0, zmax: 100 },
        st: { x: { min: 21, max: 1151, range: 1130 }, y: { min: -378, max: 752, range: 1130 }, z: { min: 22, max: 249, range: 227 } },
    }
    it(`should normalize with scale when interp2Normalize(scale=100)`, function() {
        k = 1
        let r = interp2Normalize(oin[k].ps, oin[k].opt)
        delete r.nv
        delete r.inv
        assert.strict.deepStrictEqual(r, out[k])
    })

    k++
    oin[k] = { ps: [{ x: 243, y: 206, z: 95, ind: 101 }, { x: 233, y: 225, z: 146, ind: 102 }, { x: 21, y: 325, z: 22, ind: 103 }, { x: 953, y: 28, z: 223, ind: 104 }, { x: 1092, y: 290, z: 39, ind: 105 }, { x: 744, y: 200, z: 191, ind: 106 }, { x: 174, y: 3, z: 22, ind: 107 }, { x: 537, y: 368, z: 249, ind: 108 }, { x: 1151, y: 371, z: 86, ind: 109 }, { x: 814, y: 252, z: 125, ind: 110 }], opt: { keyInd: 'ind' } }
    out[k] = {
        ps: [{ x: 0.19646017699115045, y: 0.5168141592920354, z: 0.32158590308370044, ind: 101 }, { x: 0.18761061946902655, y: 0.5336283185840708, z: 0.5462555066079295, ind: 102 }, { x: 0, y: 0.6221238938053097, z: 0, ind: 103 }, { x: 0.8247787610619469, y: 0.35929203539823007, z: 0.8854625550660793, ind: 104 }, { x: 0.947787610619469, y: 0.5911504424778761, z: 0.07488986784140969, ind: 105 }, { x: 0.6398230088495576, y: 0.511504424778761, z: 0.7444933920704846, ind: 106 }, { x: 0.13539823008849558, y: 0.33716814159292036, z: 0, ind: 107 }, { x: 0.45663716814159294, y: 0.6601769911504425, z: 1, ind: 108 }, { x: 1, y: 0.6628318584070797, z: 0.28193832599118945, ind: 109 }, { x: 0.7017699115044248, y: 0.5575221238938053, z: 0.45374449339207046, ind: 110 }],
        psMinMax: { xmin: 0, xmax: 1, ymin: 0.33716814159292036, ymax: 0.6628318584070797, zmin: 0, zmax: 1 },
        st: { x: { min: 21, max: 1151, range: 1130 }, y: { min: -378, max: 752, range: 1130 }, z: { min: 22, max: 249, range: 227 } },
    }
    it(`should preserve index field when interp2Normalize(keyInd='ind')`, function() {
        k = 2
        let r = interp2Normalize(oin[k].ps, oin[k].opt)
        delete r.nv
        delete r.inv
        assert.strict.deepStrictEqual(r, out[k])
    })

})
