...
function interp2NaturalNeighbor(psSrc, psTar, opt = {}) {
    ...
    //iniVor
    let iniVor = intDny.voronoi()

    //addPointCore
    let addPointCore = (data, x, y) => {

        //isPointInPolygon
        if (!isPointInPolygon(hpg, [x, y])) {
            return null
        }
        ...
        //newItem
        let newItem = { x, y, z: -1 } //已經被正規化至x,y,z
        ...
        //postVor
        let postVor = Delaunay.from(psSrcTemp, d => d.x, d => d.y).voronoi() //已經被正規化至x,y
